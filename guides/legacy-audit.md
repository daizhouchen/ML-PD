...
[文档目录](README.md) · [项目首页](../README.md) · [历史文件索引](../archive/README.md)

审查基线：20275fb。历史文件保留供溯源，新版入口为 mlpd CLI。新旧流程不同，分数不能直接用来计算改善率。

| 发现 | 源码证据 | 新版处理 |
|---|---|---|
| 全量筛选先于测试划分 | [原 RFE.ipynb](../archive/notebooks/feature-selection/three-class.ipynb) 的 RFECV.fit(X,y) 在 train_test_split(U,y) 前 | 全部预处理与 RFE 进入内层训练 Pipeline |
| precision/recall 语义颠倒 | [原 ML.ipynb](../archive/notebooks/classification/three-class.ipynb) 手工函数对行和、列和的解释颠倒 | sklearn classification_report；行真实、列预测 |
